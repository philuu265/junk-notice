...
function replaceIncludes(file, fileType, returnType) {
  ...
  return function (match, startBlock, spacing, blockType, oldScripts, endBlock, offset, string) {
    ...
    (string.substr(0, offset) + string.substr(offset + match.length)).replace(oldScripts, '').replace(fileType.block, '').replace(fileType.detect[blockType], function (match, reference) {
    ...
    dependencies.map(function (filePath) {
      return $.path.join(
